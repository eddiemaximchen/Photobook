...
from photobook.function import confirm_required, permission_required,db,push_comment_notification,push_collect_notification,rename_image,resize_image,flash_errors,redirect_back
from photobook.form import DescriptionForm, TagForm, CommentForm
from photobook.models import User, Photo, Tag, Follow, Collect, Comment, Notification
...
def index():
    if current_user.is_authenticated:
        ...
        per_page = current_app.config['photobook_PHOTO_PER_PAGE']
...
def search():
    ...
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['photobook_SEARCH_RESULT_PER_PAGE']
    if category == 'user':
...
def show_notifications():
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['photobook_NOTIFICATION_PER_PAGE']
...
def get_image(filename):
    return send_from_directory(current_app.config['photobook_UPLOAD_PATH'], filename)
...
def upload():
    if request.method == 'POST' and 'file' in request.files:
        ...
        f.save(os.path.join(current_app.config['photobook_UPLOAD_PATH'], filename))
        filename_s = resize_image(f, filename, current_app.config['photobook_PHOTO_SIZE']['small'])
        filename_m = resize_image(f, filename, current_app.config['photobook_PHOTO_SIZE']['medium'])
        photo = Photo(
            filename=filename,
...
def show_photo(photo_id):
    ...
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['photobook_COMMENT_PER_PAGE']
...
def show_collectors(photo_id):
    photo = Photo.query.get_or_404(photo_id)
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['photobook_USER_PER_PAGE']
